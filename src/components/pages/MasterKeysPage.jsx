...
export default function MasterKeysPage({ ctx }) {
  ...
  return <div className='page active'><div style={{ padding: '32px 36px' }}>
    ...
    ) : !masterKeys.length ? <div className='empty'><div className='empty-icon'>{'\u{1F511}'}</div><div className='empty-text'>No master keys configured</div><button className='btn btn-primary' onClick={() => setModal('addkey')}>Add your first key</button></div> : masterKeys.map((mk) => <div className='card' key={mk.id} style={{ display: 'flex', alignItems: 'center', gap: '16px' }}><div style={{ width: '40px', height: '40px', background: 'var(--bg4)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>{mk.provider === 'openai' ? '\u2b1b' : mk.provider === 'google' ? '\u{1F535}' : mk.provider === 'anthropic' ? '\u{1F7E7}' : mk.provider === 'deepseek' ? '\u{1F30A}' : mk.provider === 'xai' ? '\u2715' : mk.provider === 'groq' ? '\u26a1' : '\u{1F510}'}</div><div style={{ flex: 1 }}><div style={{ fontSize: '14px', fontWeight: 500, marginBottom: '3px' }}>{mk.name || providerLabel(providerOptions, mk.provider)}</div><div className='token-val' style={{ fontSize: '12px', color: 'var(--muted)' }}>{mk.key_masked}</div></div><div style={{ display:'flex', gap:'8px', alignItems:'center' }}><div style={{ fontSize: '12px', color: 'var(--dim)' }}>Added {fmtDate(mk.created_at)}</div><button className='btn btn-sm btn-danger' disabled={deletingId === mk.id} onClick={() => deleteMasterKey(mk)}>{deletingId === mk.id ? 'Deleting...' : 'Delete'}</button></div><span style={{ fontSize: '11px', background: '#2dca7215', color: 'var(--green)', padding: '3px 9px', borderRadius: '20px' }}>{providerLabel(providerOptions, mk.provider)}</span></div>)}
